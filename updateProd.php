    ...
    if(isset($_POST['update'])) {
        ...
        if(!empty($_FILES['image']['name'])){
          ...
          if ( !in_array($imageExtension, $validImageExtension) ){
            ...
            $update_query = $db_found->prepare('UPDATE `items` SET itemName=?, itemDesc=?, quantity=?, weight=?, expireDate=?, itemStatus=?, category=?, itemImage=? WHERE itemID = ?');
            ...
            $update_query = $db_found->prepare('UPDATE `items` SET itemName=?, itemDesc=?, quantity=?, weight=?, expireDate=?, itemStatus=?, category=? WHERE itemID = ?');
